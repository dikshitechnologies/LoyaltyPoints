...
export default function PointsScreen({ navigation }) {
  ...
  // Prompt on Android back only when this screen is focused
  useFocusEffect(
    useCallback(() => {
      const onBackPress = () => {
        Alert.alert(
  ...
  const convertPointsToAmount = points => {
    ...
    const onePointAMT =
      (parseFloat(currentRedeemAmount) || 0) / RedeemvalPOINT || 0;
  ...
  const validateAddMode = () => {
    ...
    if (!pointsEarned || isNaN(pointsEarned) || parseFloat(pointsEarned) <= 0) {
      errors.pointsEarned =
        'Valid points must be calculated from purchase amount';
    }
  ...
  const validateRedeemMode = () => {
    ...
    if (!redeemAmount || isNaN(redeemAmount) || parseFloat(redeemAmount) <= 0) {
      errors.redeemAmount =
        'Valid amount must be calculated from redeem points';
    }
    ...
    try {
      if (mode == 'add') {
        response = await axios.delete(
          `${BASE_URL}AddPoints/deletePoints/${Id}`,
        );
      } else {
        response = await axios.delete(
          `${BASE_URL}RedeemPoints/DeleteRedeem/${Id}`,
        );
      }
    ...
    try {
      if (mode === 'add') {
        response = await axios.put(
          `${BASE_URL}AddPoints/updatePoints/${Id}`,
          payload,
        );
      } else {
        response = await axios.put(
          `${BASE_URL}RedeemPoints/UpdateRedeem/${Id}`,
          payload,
        );
      }
      if (response.status === 200) {
  ...
  const getPoints = async () => {
    ...
    if (loyaltyError) {
      setValidationErrors({
        ...validationErrors,
        [mode === 'add' ? 'addLoyaltyNumber' : 'redeemLoyaltyNumber']:
          loyaltyError,
      });
      return;
    }
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}Register/points-summary/${loyaltyNumber}/${groupCode}`,
      );
  ...
  const RedeemPoints = async () => {
    try {
      ...
      const formattedDate = `${String(todayDate.getDate()).padStart(
        2,
        '0',
      ...
      const response = await axios.post(
        `${BASE_URL}RedeemPoints/RedeemPoints`,
        payload,
      );
  ...
  const AddPointsget = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}Ratefixing/Addpointfix/${groupCode}`,
      );
      ...
      if (response.status == 200) {
        ...
        handleStatusCodeError(
          response.status,
          'Error fetching points configuration',
        );
      }
    } catch (error) {
      handleApiError(error);
  ...
  const RedeemAmount = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}Ratefixing/Redeempoints/${groupCode}`,
      );
      ...
      if (response.status == 200) {
        ...
        handleStatusCodeError(
          response.status,
          'Error fetching redeem configuration',
        );
      }
    } catch (error) {
      handleApiError(error);
  ...
  const searchCustomers = async ({
    ...
    try {
      ...
      let url;
      if (mode === 'add') {
        url = `${BASE_URL}AddPoints/SearchCustomersWithPoints/${groupCode}?searchTerm=${encodeURIComponent(
          term,
        )}&page=${currentPage}&pageSize=${pageSize}`;
      } else {
        url = `${BASE_URL}RedeemPoints/SearchRedeemPoints/${groupCode}?searchTerm=${encodeURIComponent(
          term,
        )}&page=${currentPage}&pageSize=${pageSize}`;
      }
  ...
  const selectCustomer = async customer => {
    if (mode === 'add') {
      ...
      const balAdded =
        parseFloat(balAmt || 0) - parseFloat(customer.points || 0);
  ...
  const renderCustomerItem = ({ item }) => (
    ...
    >
      ...
      <Text style={styles.customerDetail}>
        Loyalty: {item.loyaltyNumber || 'N/A'}
      </Text>
      <Text style={styles.customerDetail}>
        Date:{' '}
        {item.lDate
          ? new Date(item.lDate).toLocaleDateString('en-GB') +
            ' (' +
            new Date(item.lDate).toLocaleTimeString('en-GB', {
              hour: '2-digit',
              minute: '2-digit',
            }) +
            ')'
  ...
  const renderError = fieldName => {
    if (validationErrors[fieldName]) {
      return (
        <Text style={{ color: 'red' }}>{validationErrors[fieldName]}</Text>
      );
    }
    return null;
  };
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.tabContainer}>
                ...
                >
                  <Text
                    style={[
                      styles.tabText,
                      mode === 'redeem' && styles.activeTabText,
                    ]}
                  >
                    Redeem Points
                  </Text>
                ...
                {mode === 'add' ? (
                  ...
                  >
                    ...
                    {currentValAmount && currentValPoint
                      ? (
                          parseFloat(currentValAmount) /
                          parseFloat(currentValPoint)
                        ).toFixed(2)
                      : 'Loading...'}
                  </Text>
                ) : (
                  ...
                  >
                    ...
                    {currentRedeemAmount && currentRedeemPoint
                      ? (
                          parseFloat(currentRedeemAmount) /
                          parseFloat(currentRedeemPoint)
                        ).toFixed(2)
                      : 'Loading...'}
                  </Text>
                )}
              ...
              <View style={styles.card}>
                ...
                <View style={styles.row}>
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      hasPermission ? setShowScanner(true) : requestPermission()
                    }
                ...
                {mode === 'add' ? (
                  <>
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        validationErrors.purchaseAmount && styles.inputError,
                      ]}
                    ...
                    <TextInput
                      style={[
                        styles.input,
                        validationErrors.pointsEarned && styles.inputError,
                      ]}
                      value={pointsEarned}
                      editable={false}
                    />
                  ...
                  <>
                    ...
                    <TextInput
                      ...
                      style={[
                        styles.input,
                        validationErrors.redeemPoints && styles.inputError,
                      ]}
                      value={redeemPoints}
                      onChangeText={val => {
                        setRedeemPoints(val);
                    ...
                    <TextInput
                      style={[
                        styles.input,
                        validationErrors.redeemAmount && styles.inputError,
                      ]}
                      value={redeemAmount}
                      editable={false}
                      keyboardType="phone-pad"
                ...
                <TextInput
                  style={[
                    styles.input,
                    validationErrors[
                      mode === 'add' ? 'addNarration' : 'redeemNarration'
                    ] && styles.inputError,
                    { height: 80, textAlignVertical: 'top' },
                  ]}
                  value={mode === 'add' ? addNarration : redeemNarration}
                  onChangeText={
                    mode === 'add' ? setAddNarration : setRedeemNarration
                  }
                  multiline={true}
                  numberOfLines={4}
                  returnKeyType="done"
