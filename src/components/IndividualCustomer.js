...
const IndividualCustomer = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Loyalty Number Input + QR Scanner */}
      <View style={styles.inputRow}>
        ...
        >
          <MaterialIcons name="qr-code-scanner" size={hp('3%')} color="#fff" />
        </TouchableOpacity>
      </View>
        ...
        renderItem={({ item }) => (
          <View style={styles.transactionItem}>
            ...
            <View style={styles.typeContainer}>
              ...
              <Text style={[styles.transactionType, item.type === 'CR' ? styles.creditType : styles.debitType]}>{item.type}</Text>
            </View>
      ...
      {!customerData && !loading && (
        <View style={styles.emptyStateContainer}>
          ...
          <Text style={styles.emptyStateText}>Scan a loyalty QR code or enter loyalty number</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: hp('2.5%'), fontWeight: 'bold', color: '#006A72', margin: hp('2%') },
  inputRow: { flexDirection: 'row', marginHorizontal: wp('4%'), marginBottom: hp('2%') },
  input: { flex: 1, borderWidth: 1, borderColor: '#ccc', borderRadius: 5, paddingHorizontal: wp('2%'), height: hp('6%') },
  scannerButton: { backgroundColor: '#006A72', justifyContent: 'center', alignItems: 'center', paddingHorizontal: wp('4%'), borderRadius: 5, marginLeft: wp('2%') },
  closeButton: { backgroundColor: '#006A72', padding: hp('2%'), borderRadius: 5, alignItems: 'center', marginTop: hp('2%') },
  customerInfoContainer: { padding: wp('4%'), alignItems: 'center' },
  customerCard: { borderRadius: wp('4%'), marginVertical: hp('2%'), width: '90%', height: hp('20%'), elevation: 6, overflow: 'hidden' },
